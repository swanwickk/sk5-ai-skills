...
def mk_unit(id_num_str, name, unit_type, x, y, side="Red", track_num=1000,
    ...
    if not is_installation:
        ...
        unit["Range"] = -100000
        unit["WpDistance"] = 0
        unit["PastWaypointArray"] = {}
        unit["FutureWaypointArray"] = {}
    unit["TextTags"] = {
        "TagAltitude": False,
        "TagCallsign": False,
        "TagClass": False,
...
def mk_waypoint(world_x, world_y, time_str):
    """PastWaypointArray 元素: PC对象格式（键序按字母序）"""
    return {"AltitudeDepth": 0, "Ascent": 0, "AssignedAltDepth": 0, "Course": 0,
            "Descent": 0, "IsTurnTime": True, "Name": "", "Number": 1,
            "PositionTime": time_str, "Speed": 0,
            "X": int(round(world_x)), "Y": int(round(world_y))}
...
def write_scenario_save(data, out_path):
    """写出场景存档（PC格式：紧凑+sort_keys+单行+CRLF换行，见SKILL §3序列化铁律）"""
