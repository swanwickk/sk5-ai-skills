...
description: "SK5战舰冷数据库结构、CLI极速检索与推演调船手册。"
version: 1.1.0
author: Hermes Agent
license: MIT
...
在 Linux 终端或 AI 执行环境中调用（任意 Python 3.10+ 环境，无需第三方依赖）：

```bash
# 1. 查看数据库统计与各舰型/编制分布
python3 ./database/sk5_db.py stats

# 2. 列出战舰列表（支持按国别、舰型、编制过滤）
python3 ./database/sk5_db.py list --country "UNITED STATES" --hull BB
python3 ./database/sk5_db.py list --country "UNITED STATES" --hull CV
python3 ./database/sk5_db.py list --team "美军驱逐舰_WW2"

# 3. 调出指定战舰
# (1) 战舰快速属性卡
python3 ./database/sk5_db.py get "爱荷华"
python3 ./database/sk5_db.py get "Enterprise (CV 6)"
python3 ./database/sk5_db.py get "Farragut (TB11)"

# (2) 提取完整标准 Markdown 船表
python3 ./database/sk5_db.py get "Alaska" --format md

# (3) 提取计算器 JSON 速填卡（直接粘贴至 sk5.swanwick.site）
python3 ./database/sk5_db.py get "艾尔文" --format json

# (4) 导出船表至指定工作区文件
python3 ./database/sk5_db.py get "Washington" --format md --out ./Washington_1942.md

# 4. 全文关键词检索（武器、雷达、飞机、动力型号）
python3 ./database/sk5_db.py search "16\"/50 Mk 7"
python3 ./database/sk5_db.py search "Kingfisher"
python3 ./database/sk5_db.py search "CXAM"

# 5. 复合数值条件筛选
python3 ./database/sk5_db.py query --min-gun 16.0 --min-dp 3000 --country "UNITED STATES"
python3 ./database/sk5_db.py query --radar --min-speed 30.0
```
